  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1120px]">
          <p className="text-center text-[15px] font-medium tracking-[-0.025em] text-white/85 light:text-[#000339]/85">
        ...
        <div className="mx-auto mt-7 grid max-w-[950px] grid-cols-2 items-center gap-x-8 gap-y-8 text-center sm:grid-cols-3 lg:grid-cols-6">
          {partners.map((partner, index) => (
            <motion.div
              ...
              className={`flex items-center justify-center gap-1.5 text-[15px] font-semibold tracking-[-0.055em] text-white/75 light:text-[#000339]/70 ${
                partner.name === "NOMAD" ? "tracking-[0.05em]" : ""
              }`}
            >
              {partner.mark && (
                <span className="text-[14px] leading-none text-white/65 light:text-[#000339]/60">{partner.mark}</span>
              )}
              {partner.name}
            </motion.div>
          ))}
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="absolute left-0 top-1/2 z-20 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/10 text-white/50 transition hover:border-[#3484ff]/60 hover:text-white light:border-[#000339]/15 light:text-[#000339]/50 light:hover:text-[#000339] lg:grid"
          >
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.1, backgroundColor: "rgba(52, 132, 255, 0.18)" }}
            whileTap={{ scale: 0.95 }}
            className="absolute right-0 top-1/2 z-20 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/10 text-white/50 transition hover:border-[#3484ff]/60 hover:text-white light:border-[#000339]/15 light:text-[#000339]/50 light:hover:text-[#000339] lg:grid"
          >
                ...
                return (
                  <motion.article
                    ...
                    className={`group flex h-[262px] w-[195px] shrink-0 cursor-pointer flex-col overflow-hidden rounded-[18px] border px-5 py-5 text-left backdrop-blur-md transition-[border-color,box-shadow] duration-300 hover:border-[#80b4ff]/80 hover:shadow-[0_18px_46px_rgba(22,72,255,0.35)] ${
                      isActive
                        ? "active-case-card border-[#80b4ff]/45 bg-[linear-gradient(145deg,rgba(22,72,255,0.3),rgba(0,3,57,0.72))] shadow-[0_14px_42px_rgba(22,72,255,0.28)] light:border-[#1648ff]/40 light:bg-[linear-gradient(145deg,rgba(52,132,255,0.18),rgba(255,255,255,0.92))] light:shadow-[0_14px_42px_rgba(22,72,255,0.18)]"
                        : "border-white/[0.1] bg-[linear-gradient(145deg,rgba(52,132,255,0.13),rgba(0,3,57,0.58))] light:border-[#000339]/10 light:bg-[linear-gradient(145deg,rgba(52,132,255,0.1),rgba(255,255,255,0.85))]"
                    }`}
                  >
                    ...
                    <h3 className="translate-y-0 text-[16px] font-medium tracking-[-0.04em] text-white transition-transform duration-300 ease-out group-hover:-translate-y-0.5 light:text-[#000339]">{useCase.title}</h3>
                    {isActive && (
                      <p className="mt-2 text-[12px] leading-[1.45] text-white/70 light:text-[#000339]/70">{useCase.description}</p>
                    )}
                  </motion.article>
                );
              })}
            ...
            {useCases.map((useCase, index) => (
              <button
                ...
                className={`h-1.5 w-1.5 rounded-full transition ${
                  activeCard === index
                    ? "bg-[#80b4ff] light:bg-[#1648ff]"
                    : "bg-white/25 hover:bg-white/55 light:bg-[#000339]/20 light:hover:bg-[#000339]/45"
                }`}
              />
            ))}
          </div>
